...
gene_names = append(gene_names, "Six5")
...
other_cols = jt.cols[jt.cols %notin% wjt.cols]
for (i in 1:length(other_cols)){
  other_cols[i] = "#D3D3D3"
  wjt.cols = append(wjt.cols, other_cols[i])
}

jt.PT = Waterfall.Pseudotime(jt, angle=180, col = wjt.cols, plot_title = " Adult", nclust = 5, seed = 5, invert_pt = T,scalePCA = F,lines=F,mst=F)
